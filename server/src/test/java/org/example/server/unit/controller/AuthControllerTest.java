...
class AuthControllerTest {
    ...
    @Test
    @DisplayName("HTTP 200 OK: Successfully creates new user during Google login")
    void googleLogin_CreatesNewUser_WhenUserDoesNotExist() {
        // Arrange
        Map<String, String> payload = Map.of(
        ...
        // First findByEmail returns null (user doesn't exist), second returns the newly
        // created user
        when(userRepository.findByEmail("newuser@gmail.com"))
                .thenReturn(null)
                .thenReturn(newUser);
        when(newUser.getDetail(newUser)).thenReturn(userDetails);
        when(tokenProvider.generateToken("newuser@gmail.com", AccountType.Google.ordinal()))
                .thenReturn("google-dummy-token");
        ...
        verify(authService).saveUser(any(UserDto.class));
        verify(tokenProvider).generateToken("newuser@gmail.com", AccountType.Google.ordinal());
    }

    @Test
    @DisplayName("HTTP 500 INTERNAL_SERVER_ERROR: Returns error when Google login fails")
    void googleLogin_ReturnsError_WhenExceptionOccurs() {
        // Arrange
        Map<String, String> payload = Map.of(
                "email", "test@gmail.com",
                "name", "Google User");

        // Simulate an exception during email retrieval
        when(userRepository.findByEmail("test@gmail.com")).thenThrow(new RuntimeException("Database error"));
        ...
        assertTrue(response.getBody().getError().getMessage().contains("Google login failed"));
    }
    ...
    @Test
    @DisplayName("HTTP 500 INTERNAL_SERVER_ERROR: Returns error when sending enquiry fails")
    void sendEnquiry_ReturnsError_WhenExceptionOccurs() {
        // Arrange
        EnquiryRequestDto requestDto = new EnquiryRequestDto("Test Name", "test@example.com", "Test Subject",
                "Test Body");

        // Simulate an exception during saving
        when(enquiryRepository.save(any(Enquiry.class))).thenThrow(new RuntimeException("Database error"));
        ...
        assertNull(response.getBody().getResponse());
        assertNotNull(response.getBody().getError());
        assertEquals("ENQUIRY_ERROR", response.getBody().getError().getCode());
        assertTrue(response.getBody().getError().getMessage().contains("Send enquiry failed"));
    }
    ...
    @Test
    @DisplayName("HTTP 500 INTERNAL_SERVER_ERROR: Returns error when fetching enquiries fails")
    void getEnquiries_ReturnsError_WhenExceptionOccurs() {
        // Arrange
        // Simulate an exception during findAll
        when(enquiryRepository.findAll()).thenThrow(new RuntimeException("Database error"));
        ...
        assertNotNull(response.getBody().getError());
        assertEquals("ENQUIRY_ERROR", response.getBody().getError().getCode());
        assertTrue(response.getBody().getError().getMessage().contains("Fetching enquiry failed"));
    }

    @Test
    @DisplayName("Tests private helper methods through reflection")
    void privateHelperMethods_WorkCorrectly() throws Exception {
        // Test getUser method
        java.lang.reflect.Method getUserMethod = AuthController.class.getDeclaredMethod("getUser", String.class);
        getUserMethod.setAccessible(true);

        User expectedUser = mock(User.class);
        when(userRepository.findByEmail("test@example.com")).thenReturn(expectedUser);

        User actualUser = (User) getUserMethod.invoke(authController, "test@example.com");
        assertEquals(expectedUser, actualUser);

        // Test isValidPassword method
        java.lang.reflect.Method isValidPasswordMethod = AuthController.class.getDeclaredMethod("isValidPassword",
        ...
        Boolean validResult = (Boolean) isValidPasswordMethod.invoke(
                authController, "correctPassword", "encodedPassword");
        Boolean invalidResult = (Boolean) isValidPasswordMethod.invoke(
                authController, "wrongPassword", "encodedPassword");
