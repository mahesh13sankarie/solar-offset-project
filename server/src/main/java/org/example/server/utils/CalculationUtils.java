...
public class CalculationUtils {
    ...
    public static double calculateCarbonEmissions(ElectricityBreakdown breakdown, CarbonIntensity carbonIntensity) {
        if (breakdown.getPowerConsumptionTotal() == null || breakdown.getPowerConsumptionTotal() == 0) return 0;
    ...
    public static double calculateElectricityAvailability(ElectricityBreakdown breakdown) {
        return (breakdown.getPowerProductionTotal() + breakdown.getPowerImportTotal() - breakdown.getPowerExportTotal()) * 1000 * 24;
    }

    public static double calculateSolarPowerPotential(ElectricityBreakdown breakdown) {
        if (breakdown.getPowerProductionTotal() == null || breakdown.getPowerProductionTotal() == 0) return 0;
